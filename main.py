...
async def меню_высоких_тестов(ctx):
    ...
    embed.set_footer(text="ECL Tiers")
    await ctx.send(embed=embed, view=HighTestView())

@bot.command()
@commands.has_permissions(administrator=True)
async def меню_очереди(ctx):
...
@bot.command()
@commands.has_permissions(administrator=True)
async def управление_очередью(ctx, action: str):
    global queue_status, testing_queue
    if action == "открыть":
        queue_status = "ОТКРЫТА"
        await ctx.send("🔓 Очередь открыта!", delete_after=5)
    elif action == "закрыть":
        queue_status = "ЗАКРЫТА"
        await ctx.send("🔒 Очередь закрыта!", delete_after=5)
    elif action == "очистить":
        if testing_queue:
            testing_queue.pop(0)
            await ctx.send("🧹 Первое место удалено из очереди!", delete_after=5)
            if testing_queue:
                bot.loop.create_task(create_queue_ticket(ctx.guild))
        else:
            await ctx.send("⚠️ Очередь пуста!", delete_after=5)

    await update_queue_message()

@bot.command()
@commands.has_permissions(administrator=True)
async def очередь_пинг(ctx):
    await ctx.message.delete()
    await ctx.send("@everyone 📢 Очередь открыта! Вы можете встать в неё, нажав на кнопку «Встать в очередь✅»")

@bot.command()
async def результаты(ctx, tester_nick: str, player_nick: str, old_tier: str, new_tier: str, score: str, kit: str, member: disnake.Member):
    try:
        await ctx.message.delete()
    except Exception:
        pass
    ...
    try:
        ...
        await ctx.send(f"❌ Произошла ошибка при отправке результатов:\n```text\n{e}\n```")
...
async def топ(ctx):
    ...
    """, (ctx.channel.id, new_msg.id))
    conn.commit()
    conn.close()

@bot.command()
@commands.has_permissions(administrator=True)
async def топ_игрок(ctx, member: disnake.Member, display_name: str, points: int, *, kits: str):
    try:
        await ctx.message.delete()
    except:
        pass
    ...
    refreshed = await auto_refresh_top(ctx.bot)
    status = "Топ обновлен прямо в чате!" if refreshed else "Сохранено. Напишите !топ для создания карточки."
    await ctx.send(f"✅ Данные игрока **{display_name}** изменены. {status}", delete_after=5)

@bot.command()
@commands.has_permissions(administrator=True)
async def топ_удалить(ctx, member: disnake.Member):
    try:
        await ctx.message.delete()
    except:
        pass
    ...
    await ctx.send(f"🧹 Игрок удален. {status}", delete_after=5)
